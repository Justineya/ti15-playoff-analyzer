...
1. 打开 `web/data/player.json`。数字以这份为准，不要编比分、KDA、分位。解析场有 `job`：技能次数、跳刀/支配使用、10/15/20 团队经济、全队塔伤、偷技能。没有就去 OpenDota 比赛详情补 `ability_uses` / `item_uses` / `gold_t` / `teamfights`。
...
看本职，不要用塔伤当所有英雄的尺子。先问这把英雄该干什么，再挑指标：
- 黑贤：壳 / 涌动 / 真空 / 墙 / 支配刷 / 跳刀次数。不是拆塔。
- 光法：照亮 / 查克拉 / 大骨灰。塔是一号或狼人的。
- 拉比克：偷了什么、团战参与、助攻分位。不是拆塔。
- 狼人、一号位、召唤拆家流：才看塔伤。
- 全队塔伤 0 是地图没人拆，不要写成「黑贤塔 0 所以没收」。

分类：7k+ 位置胜率 ≤47% → meta_weak；本职含拆塔的英雄 GPM≥80% 且塔<50% 仍负 → did_not_close；光环/偷技能/先手英雄 GPM 高但团战<45%（技能几乎没打）才算没收；一号位 GPM <20% → wrong_role。
